<template>
  <div>
    ...
    <el-button
      type="primary"
      @click="goBack"
      style="position: absolute; left: 35vh; z-index: 999999"
      v-if="status&&this.$store.state.code != '320000'"
      >返回上级</el-button
    >
    <div
      ref="mapChartRef"
      style="height: 55vh; width: 51vh"
      id="mapChartRef"
    ></div>
    <!-- </el-card> -->
  </div>
      ...
      const option = {
        ...
        tooltip: {
          ...
          formatter: (params) => {
            ...
            if (params.data.tempLevelname == "中等风险") {
              str += `<div style="text-align:left;">预警等级：<span style="color:rgb(0,0,255)">${params.data.tempLevelname}</span></div>`;
            } else if (params.data.tempLevelname == "较高风险") {
              str += `<div style="text-align:left;">预警等级：<span style="color:rgb(255,255,0)">${params.data.tempLevelname}</span></div>`;
            } else if (params.data.tempLevelname == "高风险") {
              str += `<div style="text-align:left;">预警等级：<span style="color:rgb(255,126,0)">${params.data.tempLevelname}</span></div>`;
            } else if (params.data.tempLevelname == "极高风险") {
              str += `<div style="text-align:left;">预警等级：<span style="color:rgb(255,0,0)">${params.data.tempLevelname}</span></div>`;
            } else {
              str += `<div style="text-align:left">预警等级：${params.data.tempLevelname}</div>`;
            }

            str += `<div style="text-align:left">最低温度：${params.data.minTemp}</div>
            <div style="text-align:left">最高温度：${params.data.highTemp}</div>
            <div style="text-align:left">平均温度：${params.data.avgTemp}</div>`;
        ...
        visualMap: {
          ...
          pieces: [
            ...
            { min: 0, max: 0, label: "低风险", color: "#69C0FF" },
            { min: 1, max: 1, label: "中等风险", color: "rgb(0,0,255)" },
            { min: 2, max: 2, label: "较高风险", color: "rgb(255,255,0)" },
            { min: 3, max: 3, label: "高风险", color: "rgb(255,126,0)" },
            { min: 4, max: 4, label: "极高风险", color: "rgb(255,0,0)" },
          ],
        },
      };
